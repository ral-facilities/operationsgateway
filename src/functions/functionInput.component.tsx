...
import {
  ...
  // operators,
  // parseFilter,
} from '../filtering/filterParser';
import { useFunctionReturnType } from '../api/functions';
...
const FunctionInput = (props: FunctionInputProps) => {
  ...
  const options = React.useMemo(() => {
    return [...backendTokens, ...channels];
  }, [backendTokens, channels]);
  const [inputValue, setInputValue] = React.useState<string>('');
  // on load, set input position to the end
  const [inputIndex, setInputIndex] = React.useState<number>(value.length);
  let errorMessage = '';
  try {
    const returnType = useFunctionReturnType(index, functionItems);
    functionItems[index].returnType = returnType.data;
  } catch (e) {
  ...
  const keydownHandler = React.useCallback<(e: React.KeyboardEvent) => void>(
    ...
    [inputValue, value, setValue, setError, inputIndex, channels, backendTokens]
  );
