export const prefix = 'data-rbd';
export const dragHandle = (() => {
  const base = `${prefix}-drag-handle`;

  return {
    base,
    draggableId: `${base}-draggable-id`,
    contextId: `${base}-context-id`,
  };
})();

export const draggable = (() => {
  const base = `${prefix}-draggable`;
  return {
    base,
    contextId: `${base}-context-id`,
    id: `${base}-id`,
  };
})();

export const droppable = (() => {
  const base = `${prefix}-droppable`;
  return {
    base,
    contextId: `${base}-context-id`,
    id: `${base}-id`,
  };
})();

export const placeholder = {
  contextId: `${prefix}-placeholder-context-id`,
};

export const scrollContainer = {
  contextId: `${prefix}-scroll-container-context-id`,
};

export function getDroppableSelector(droppableId) {
  if (droppableId) {
    return `[${droppable.id}="${droppableId}"]`;
  }
  return `[${droppable.id}]`;
}

export function getHandleSelector(draggableId) {
  if (draggableId) {
    return `[${dragHandle.draggableId}="${draggableId}"]`;
  }
  return `[${dragHandle.draggableId}]`;
}

export function getDraggableSelector(draggableId) {
  if (draggableId) {
    return `[${draggable.id}="${draggableId}"]`;
  }
  return `[${draggable.id}]`;
}

export const testRecordResponse = [
  {
    _id: '1',
    metadata: {
      shotnum: 1,
      timestamp: '2022-01-01T00:00:00',
    },
    channels: {
      CHANNEL_ABCDE: {
        metadata: {
          channel_dtype: 'scalar',
        },
        data: 1,
      },
    },
  },
  {
    _id: '2',
    metadata: {
      shotnum: 2,
      timestamp: '2022-01-02T00:00:00',
    },
    channels: {
      CHANNEL_ABCDE: {
        metadata: {
          channel_dtype: 'scalar',
        },
        data: 2,
      },
    },
  },
  {
    _id: '3',
    metadata: {
      shotnum: 3,
      timestamp: '2022-01-03T00:00:00',
    },
    channels: {
      CHANNEL_ABCDE: {
        metadata: {
          channel_dtype: 'scalar',
        },
        data: 3,
      },
    },
  },
  {
    _id: '4',
    metadata: {
      shotnum: 4,
      timestamp: '2022-01-04T00:00:00',
    },
    channels: {
      CHANNEL_BCDEF: {
        metadata: {
          channel_dtype: 'image',
          x_pixel_size: 100,
          x_pixel_units: 'nm',
          v_pixel_size: 100,
          v_pixel_units: 'nm',
          gain: 5.0,
          exposure_time_s: 0.001
        },
        image_path: '/path/to/image.png',
        thumbnail: '',
      },
    },
  },
  {
    _id: '5',
    metadata: {
      shotnum: 5,
      timestamp: '2022-01-05T00:00:00',
    },
    channels: {
      CHANNEL_BCDEF: {
        metadata: {
          channel_dtype: 'image',
          x_pixel_size: 100,
          x_pixel_units: 'nm',
          v_pixel_size: 100,
          v_pixel_units: 'nm',
          gain: 5.0,
          exposure_time_s: 0.001
        },
        image_path: '/path/to/image.png',
        thumbnail: '',
      },
    },
  },
  {
    _id: '6',
    metadata: {
      shotnum: 6,
      timestamp: '2022-01-06T00:00:00',
    },
    channels: {
      CHANNEL_BCDEF: {
        metadata: {
          channel_dtype: 'image',
          x_pixel_size: 100,
          x_pixel_units: 'nm',
          v_pixel_size: 100,
          v_pixel_units: 'nm',
          gain: 5.0,
          exposure_time_s: 0.001
        },
        image_path: '/path/to/image.png',
        thumbnail: '',
      },
    },
  },
  {
    _id: '7',
    metadata: {
      shotnum: 7,
      timestamp: '2022-01-07T00:00:00',
    },
    channels: {
      CHANNEL_CDEFG: {
        metadata: {
          channel_dtype: 'waveform',
          x_units: 'x',
          y_units: 'y'
        },
        waveform_id: '7',
        thumbnail: '',
      },
    },
  },
  {
    _id: '8',
    metadata: {
      shotnum: 8,
      timestamp: '2022-01-08T00:00:00',
    },
    channels: {
      CHANNEL_CDEFG: {
        metadata: {
          channel_dtype: 'waveform',
          x_units: 'x',
          y_units: 'y'
        },
        waveform_id: '8',
        thumbnail: '',
      },
    },
  },
  {
    _id: '9',
    metadata: {
      shotnum: 9,
      timestamp: '2022-01-09T00:00:00',
    },
    channels: {
      CHANNEL_CDEFG: {
        metadata: {
          channel_dtype: 'waveform',
          x_units: 'x',
          y_units: 'y'
        },
        waveform_id: '9',
        thumbnail: '',
      },
    },
  },
  {
    _id: '10',
    metadata: {
      shotnum: 10,
      timestamp: '2022-01-10T00:00:00',
    },
    channels: {
      CHANNEL_DEFGH: {
        metadata: {
          channel_dtype: 'scalar',
        },
        data: 10,
      },
    },
  },
  {
    _id: '11',
    metadata: {
      shotnum: 9,
      timestamp: '2022-01-11T00:00:00',
    },
    channels: {
      CHANNEL_DEFGH: {
        metadata: {
          channel_dtype: 'scalar',
        },
        data: 11,
      },
    },
  },
  {
    _id: '12',
    metadata: {
      shotnum: 12,
      timestamp: '2022-01-12T00:00:00',
    },
    channels: {
      CHANNEL_DEFGH: {
        metadata: {
          channel_dtype: 'scalar',
        },
        data: 12,
      },
    },
  },
  {
    _id: '13',
    metadata: {
      shotnum: 13,
      timestamp: '2022-01-13T00:00:00',
    },
    channels: {
      CHANNEL_EFGHI: {
        metadata: {
          channel_dtype: 'image',
          x_pixel_size: 100,
          x_pixel_units: 'nm',
          v_pixel_size: 100,
          v_pixel_units: 'nm',
          gain: 5.0,
          exposure_time_s: 0.001
        },
        image_path: '/path/to/image.png',
        thumbnail: '',
      },
    },
  },
  {
    _id: '14',
    metadata: {
      shotnum: 14,
      timestamp: '2022-01-14T00:00:00',
    },
    channels: {
      CHANNEL_EFGHI: {
        metadata: {
          channel_dtype: 'image',
          x_pixel_size: 100,
          x_pixel_units: 'nm',
          v_pixel_size: 100,
          v_pixel_units: 'nm',
          gain: 5.0,
          exposure_time_s: 0.001
        },
        image_path: '/path/to/image.png',
        thumbnail: '',
      },
    },
  },
  {
    _id: '15',
    metadata: {
      shotnum: 15,
      timestamp: '2022-01-15T00:00:00',
    },
    channels: {
      CHANNEL_EFGHI: {
        metadata: {
          channel_dtype: 'image',
          x_pixel_size: 100,
          x_pixel_units: 'nm',
          v_pixel_size: 100,
          v_pixel_units: 'nm',
          gain: 5.0,
          exposure_time_s: 0.001
        },
        image_path: '/path/to/image.png',
        thumbnail: '',
      },
    },
  },
  {
    _id: '16',
    metadata: {
      shotnum: 16,
      timestamp: '2022-01-16T00:00:00',
    },
    channels: {
      CHANNEL_FGHIJ: {
        metadata: {
          channel_dtype: 'waveform',
          x_units: 'x',
          y_units: 'y'
        },
        waveform_id: '16',
        thumbnail: '',
      },
    },
  },
  {
    _id: '17',
    metadata: {
      shotnum: 17,
      timestamp: '2022-01-17T00:00:00',
    },
    channels: {
      CHANNEL_FGHIJ: {
        metadata: {
          channel_dtype: 'waveform',
          x_units: 'x',
          y_units: 'y'
        },
        waveform_id: '17',
        thumbnail: '',
      },
    },
  },
  {
    _id: '18',
    metadata: {
      shotnum: 18,
      timestamp: '2022-01-18T00:00:00',
    },
    channels: {
      CHANNEL_FGHIJ: {
        metadata: {
          channel_dtype: 'waveform',
          x_units: 'x',
          y_units: 'y'
        },
        waveform_id: '18',
        thumbnail: '',
      },
    },
  },
];

export const testRecordCountResponse = testRecordResponse.length;
